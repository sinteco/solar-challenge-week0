#!/usr/bin/env python3
"""
Perform EDA for Sierra Leone dataset.

Outputs:
 - cleaned CSV -> data/sierraleone_clean.csv
 - report text -> outputs/sierraleone/report.txt
 - plots -> outputs/sierraleone/*.png

This script is idempotent and will create the outputs directory if missing.
"""
import os
...
def ensure_dirs():
    ...
    Path("outputs/sierraleone").mkdir(parents=True, exist_ok=True)


def load_data():
    candidates = [
        Path("data/sierraleone.csv"),
        Path("data/sierraleone_clean.csv"),
        Path("notebooks/data/sierraleone_clean.csv"),
        Path("notebooks/data/sierraleone.csv"),
    ]
    for p in candidates:
        if p.exists():
            print(f"Loading {p}")
            ...
            return df
    raise FileNotFoundError("No sierraleone CSV found in expected locations. Put raw data at data/sierraleone.csv")
...
def flag_outliers_zscore(df, cols):
    ...
    for c in cols:
        if c in df.columns and pd.api.types.is_numeric_dtype(df[c]):
            ...
            # create a column for this outlier flag
            mask = False
            if len(z) > 0:
...
def save_report(text, filename="outputs/sierraleone/report.txt"):
    with open(filename, "w") as f:
        f.write(text)


def make_plots(df):
    outdir = Path("outputs/sierraleone")
    # Ensure Timestamp is datetime
    if "Timestamp" in df.columns:
        df["Timestamp"] = pd.to_datetime(df["Timestamp"], errors="coerce")
        df = df.sort_values("Timestamp")

    # Time series: GHI, DNI, DHI, Tamb
    ...
    # Wind rose (simple polar bar) using WD (degrees) and WS
    ...
    # Bubble chart GHI vs Tamb sized by RH or BP
...
def main():
    ...
    # Save cleaned
    clean_path = Path("data/sierraleone_clean.csv")
    ...
    # Create text report
    lines = []
    lines.append("SierraLeone EDA Report\n")
