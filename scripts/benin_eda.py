#!/usr/bin/env python3
"""
Perform EDA for Benin dataset.

Outputs:
 - cleaned CSV -> data/benin_clean.csv
 - report text -> outputs/benin/report.txt
 - plots -> outputs/benin/*.png
...
def ensure_dirs():
    ...
    Path("outputs/benin").mkdir(parents=True, exist_ok=True)
...
def load_data():
    candidates = [
        ...
        Path("notebooks/data/benin_clean.csv"),
        Path("notebooks/data/benin.csv"),
    ]
    for p in candidates:
        if p.exists():
            print(f"Loading {p}")
            ...
            return df
    raise FileNotFoundError("No benin CSV found in expected locations. Put raw data at data/benin.csv")
...
def save_report(text, filename="outputs/benin/report.txt"):
    with open(filename, "w") as f:
        f.write(text)


def make_plots(df):
    outdir = Path("outputs/benin")
...
def main():
    ...
    # Save cleaned
    clean_path = Path("data/benin_clean.csv")
